...
- [ ] **記事のデフォルト画像設定**
  - 記事にheroImageが設定されていない場合のデフォルト画像を用意
  - 現在の実装：`src/layouts/Layout.astro`で`/og-image.jpg`を参照しているが、ファイルが存在しない
  - 実装方法：
    - OGP推奨サイズ（1200x630px）のデフォルト画像を作成
    - `public/og-image.jpg`として配置
    - または既存の`profile.png`を使用するようコードを変更
  - 対象ファイル：`src/layouts/Layout.astro`（14行目）
